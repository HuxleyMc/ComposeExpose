...
When refreshing, ComposeExpose uses the nearest `gradlew` found at `--project-root` or one of its parents. That keeps nested demo or included builds refreshable even when the wrapper lives at the repository root.
