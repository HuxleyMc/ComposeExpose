...
abstract class ComposeExposeIndexTask : DefaultTask() {
    ...
    fun writeIndex() {
        ...
        val index = when (backend.get().lowercase()) {
            "source" -> {
            ...
            "ksp" -> {
                val generatedIndexes = kspIndexFiles.files
                    .filter { it.exists() }
            ...
            else -> throw GradleException("Unsupported ComposeExpose backend '${backend.get()}'. Use 'source' or 'ksp'.")
        }
